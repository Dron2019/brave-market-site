export function galleryAnimation() {
  ...
  const sliderWidth = $sliderImages.reduce((acc, el) => {
    const margin = getComputedStyle(el).marginLeft ? parseInt(getComputedStyle(el).marginLeft) : 0;
    console.log(margin);
  ...
  let scrollTween = gsap
    ...
    .fromTo(
      '[data-gallery-scene] .home-gallery__slider',
      {
  ...
  if (document.documentElement.classList.contains('desktop')) {
    gsap
      ...
      .fromTo(
        '.home-gallery__content>.button-30',
        {
          autoAlpha: 0,
        },
        {
